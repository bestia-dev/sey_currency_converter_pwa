...
async fn div_list_set(cursor: db::Cursor, template: &str, search_str: &str) {
    ...
    for i in 0..row_number_counter {
        ...
        row_on_click!("div_rate_", i, row_cell_on_click);
    }
    // endregion: event handlers
}
...
pub fn row_cell_on_click(_element_prefix: &str, row_number: usize) {
    ...
    let output_currency = w::get_text(&element_id);
    spawn_local(async move {
        let output_currency = output_currency.clone();
        //w::debug_write(&format!("element_id: {}", element_id));
        let currency_name = crate::currdb_currency_mod::get_name(&output_currency).await;
        crate::currdb_currency_used_mod::put_single_item(&output_currency, &currency_name).await;
